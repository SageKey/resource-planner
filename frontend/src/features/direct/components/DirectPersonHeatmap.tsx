// -------------------------------------------------------------------------
// Direct Model — person heatmap (Round 1, display-only)
// -------------------------------------------------------------------------
// Duplicated from components/capacity/PersonHeatmapGrid.tsx so the
// Direct feature can stay isolated. The v1 version calls a drill-down
// hook that hits v1's /capacity/person-heatmap-detail endpoint — that
// would return v1 data for Direct cells. Round 1 is display-only.
// -------------------------------------------------------------------------
...
import type { DirectPersonHeatmapResponse, DirectPersonRow } from "../types";
...
export function DirectPersonHeatmap({ data }: { data: DirectPersonHeatmapResponse }) {
  const [collapsedTeams, setCollapsedTeams] = useState<Set<string>>(new Set());

  const groups = useMemo<TeamGroup[]>(() => {
    const map = new Map<string, DirectPersonRow[]>();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6">
      <h2 className="mb-2 text-sm font-semibold text-slate-700">Person Heatmap</h2>
      <p className="mb-3 text-[11px] text-slate-500">
        Only people assigned to a Direct Model project show load. A
        person's cell equals the explicit weekly hours for their role in
        the current phase, divided by their project capacity.
      </p>
                          ...
                          <td className="sticky left-0 z-10 bg-white px-2 py-0.5">
                            <div className="flex items-center gap-1.5">
                              <div
                                ...
                                style={{
                                  backgroundColor: excluded ? "#e2e8f0" : avatarTone(person.name),
                                }}
                              >
                                {person.name
                                  .split(" ")
                              ...
                              <span
                                className={cn(
                                  ...
                                  excluded ? "text-slate-400 italic" : "text-slate-700",
                                )}
                              >
                                {person.name}
                              </span>
